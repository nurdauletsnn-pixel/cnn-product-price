"""
Модели: AlexNet, VGG16, GoogLeNet, ResNet50, EfficientNetB0
models/<name>/best_model.pth
...
def build_model(name: str, nc: int) -> nn.Module:
    if name == "alexnet":
        ...
        m.aux1 = None
        m.aux2 = None
        m.fc = nn.Linear(1024, nc)        # Fine-tune last inception block + fc
        layers = list(m.named_children())
        for n2, child in layers[:-3]:
            for p in child.parameters(): p.requires_grad = False
        m.fc = nn.Linear(1024, nc)

    elif name == "resnet50":
        m = models.resnet50(weights=models.ResNet50_Weights.DEFAULT)
        # Freeze layers 1-3, train layer4 + fc
        for n2, child in list(m.named_children())[:-3]:
            for p in child.parameters(): p.requires_grad = False
        m.fc = nn.Sequential(
            nn.Dropout(0.4),
            nn.Linear(2048, nc)
        ...
        for n2, child in list(m.named_children())[:-2]:
            for p in child.parameters(): p.requires_grad = False
...
# ── TRAINING ─────────────────────────────────────────────────────────────────
def train_model(model_name: str, class_names: list):
    nc = len(class_names)
    ...
    model     = build_model(model_name, nc).to(DEVICE)
    ...
    print(f"\n{'─'*58}")
    print(f"  {model_name.upper():<20} | classes={nc} | device={DEVICE}")
    print(f"{'─'*58}")
    ...
    with torch.no_grad():
        for _ in range(5): get_logits(model(dummy))   # warm-up
    ts = time.time()
    with torch.no_grad():
    ...
    sns.heatmap(cm, annot=True, fmt="d", cmap="Blues",
                ...
                annot_kws={"size":7})
    ax.set_title(f"{model_name} — Confusion Matrix (Top-1={top1:.3f})")
    ...
    axes[0].plot(ep, history["tl"], label="Train"); axes[0].plot(ep, history["vl"], label="Val")
    axes[0].set_title("Loss"); axes[0].legend(); axes[0].set_xlabel("Epoch")
    axes[1].plot(ep, history["ta"], label="Train"); axes[1].plot(ep, history["va"], label="Val")
...
# ── COMPARISON CHART ─────────────────────────────────────────────────────────
def plot_comparison(all_m):
    names = [m["model_name"] for m in all_m]
    fig, axes = plt.subplots(1,3,figsize=(16,5))

    axes[0].bar(names, [m["top1_accuracy"] for m in all_m], color="#4C72B0", label="Top-1")
    axes[0].bar(names, [m["top5_accuracy"] for m in all_m], alpha=0.5, color="#DD8452", label="Top-5")
    axes[0].set_title("Accuracy"); axes[0].set_ylim(0,1.05); axes[0].legend()
    axes[0].tick_params(axis="x", rotation=30)

    axes[1].bar(names, [m["train_time_sec"] for m in all_m], color="#55A868")
    axes[1].set_title("Train Time (sec)"); axes[1].tick_params(axis="x", rotation=30)

    axes[2].bar(names, [m["inference_ms"] for m in all_m], color="#C44E52")
...
# ── MAIN ─────────────────────────────────────────────────────────────────────
def main():
    cn_path = DATASET_DIR / "class_names.json"
    if not cn_path.exists():
        print("❌ class_names.json не найден. Сначала запустите step1_prepare.py")
        return
    with open(cn_path, encoding="utf-8") as f:
        class_names = json.load(f)["class_names"]
    print(f"📋 Классов: {len(class_names)}")

    model_list = ["alexnet","vgg16","googlenet","resnet50","efficientnet_b0"]
    all_metrics = []

    for mname in model_list:
        try:
            m = train_model(mname, class_names)
            all_metrics.append(m)
        except Exception as e:
            print(f"❌ {mname}: {e}")
            import traceback; traceback.print_exc()

    with open(MODELS_DIR/"all_metrics.json","w") as f:
    ...
    print("\n✅ Обучение завершено! Запустите: streamlit run step3_app.py")
